...
import com.campo.campocolombiano.design.constantes.Constantes2.Companion.consesion_agua
        ...
        viewModelScope.launch {
            ...
            _rsultRegister.value = withContext(Dispatchers.IO) {
                ...
                dataBase.collection("Fincas").document("Fincas")
                    .collection("ActualizacionFinca").document(Constantes2.idFincaPadre!!)
                    .update(
                        mapOf("modificaciones_fincas" to Constantes2.modificacionesFincas)
                    )

                dataBase.collection("Fincas").document("Fincas")
                    .collection("RegistroActualizacionFinca").document(Constantes2.idFincaPadre!!)
                    .collection(Constantes2.creado!!.replace("/","-"))
                    .document(Constantes2.idFincaPadre!!)
                    .set(datos_finca)
                    .addOnCompleteListener { task ->
                        if (task.isSuccessful) {
                            //se creo
                            Log.e("siSeCreo", "${task.result}")
                        }else{
                            Log.e("NOSeCreo", "${task.result}")
                        }
                    }.addOnFailureListener {
                        Log.e("ErrorCreacion", "${it.message}")
                    }
